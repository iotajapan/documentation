...
**IOTAトークンを転送する前に、IOTAトークンを転送する[バンドル](root://getting-started/0.1/transactions/bundles.md)のトランザクションを確定する必要があります。バンドル内のトランザクションは、末尾トランザクションが[マイルストーン](root://getting-started/0.1/network/the-coordinator.md)によって参照および承認されるまでペンディング状態のままです。**
